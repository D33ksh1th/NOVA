...
class WakeWordDetector:
    ...
    def detect(self, text: str) -> WakeWordResult:
        ...
        match = self._prefix().match(normalized)
    ...
    def _strip(self, text: str) -> str:
        return self._prefix().sub("", text, count=1).strip()

    def _prefix(self):
        greeting = r"(?:hey|hi|hello|ok|okay)[\s,!.:-]*"
        token = re.escape(self.wake_word)
        if self.wake_word == "nova":
            token = rf"(?:{greeting}(?:nova|novah|nowa|no\s+va|n\s+ova|lumi)|nova|lumi)"
        else:
            token = rf"(?:{greeting})?{token}"
        return re.compile(rf"^{token}\b[\s,!.:?-]*", re.IGNORECASE)
