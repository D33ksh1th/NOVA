...
class VoiceEngine:
    def __init__(
        ...
        self._voice_mode_defaults = {
            "human": {"style": "calm", "rate": 166, "pitch": 44},
    ...
    def start_enrollment_session(self, name: Optional[str] = None) -> Dict[str, Any]:
        ...
        if speaker_name:
            stage = "await_sample"
            response = (
                f"Great. I will enroll {speaker_name}. "
                f"{self._enrollment_prompts[0]}"
            )
    ...
    def _finalize_enrollment(self, session: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if audio_samples:
            profile = self.pipeline.speaker_registry.enroll_audio(
    ...
    def stop_speaking(self) -> Dict[str, Any]:
        """Best-effort stop for active TTS/audio playback."""
