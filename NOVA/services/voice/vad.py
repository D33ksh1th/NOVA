...
class SileroVoiceActivityDetector:
    ...
    def _detect_audio(self, path: Path) -> VoiceActivityResult:
        if not self._ensure_available():
            ...
            return VoiceActivityResult(True, confidence=0.5, energy=0.5, backend="silero-vad/fallback")

        try:
            return self._run_silero(path)
        except Exception as ex:
            logger.error(f"SileroVAD detection failed: {ex} — returning speech=True as safe default")
            return VoiceActivityResult(True, confidence=0.5, energy=0.5, backend="silero-vad/error")
