...
class AvatarEventStream:
    ...
    def register_bridge(self) -> None:
        ...
        bus.subscribe(Event.AVATAR_LISTENING_STARTED, lambda data: self.publish("listening_started", data))
        bus.subscribe(Event.AVATAR_LISTENING_COMPLETED, lambda data: self.publish("listening_completed", data))
        bus.subscribe(Event.AVATAR_THINKING_STARTED, lambda data: self.publish("thinking_started", data))
        bus.subscribe(Event.AVATAR_THINKING_COMPLETED, lambda data: self.publish("thinking_completed", data))
        bus.subscribe(Event.AVATAR_SPEAKING_STARTED, lambda data: self.publish("speaking_started", data))
        bus.subscribe(Event.AVATAR_SPEAKING_COMPLETED, lambda data: self.publish("speaking_completed", data))
        bus.subscribe(Event.AVATAR_IDLE, lambda data: self.publish("idle", data))
        bus.subscribe(Event.GMAIL_NEW_MESSAGE, lambda data: self.publish("gmail_new_message", data))
        bus.subscribe(Event.GMAIL_SEND_COMPLETED, lambda data: self.publish("gmail_send_completed", data))
