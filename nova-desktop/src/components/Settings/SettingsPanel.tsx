  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 py-5 border-b border-border">
        <h2 className="font-display font-bold text-xl text-text-primary">Settings</h2>
        <p className="text-text-muted text-sm mt-1">
          Configure voice, avatar, and system preferences.
        </p>
      </div>

      <div className="flex flex-1 overflow-hidden">
        {/* Section tabs */}
        <nav className="w-44 border-r border-border p-3 space-y-1 flex-shrink-0">
          {sections.map((s) => (
            <button
              key={s.id}
      ...
      <div className="border-t border-border px-6 py-4 flex items-center justify-between">
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          className="flex items-center gap-2 bg-nova-orange hover:bg-nova-glow text-white px-4 py-2 rounded-xl text-[13px] font-semibold transition-all disabled:opacity-50 shadow-glow-sm"
        >
          <Save size={14} />
    ...
    <div className="space-y-6">
      ...
      <SettingsGroup title="Voice">
        <SelectRow
          ...
          value={voice.model}
          onChange={(v) => {
            // Keep accent and style in sync with the chosen voice gender
            const isFemale = v === "english_amy";
            setVoice({
              model: v,
              accent: "english_jarvis",
              style: "calm",
              rate: 166,
          ...
          options={[
            { value: "english_lessac", label: "Male — Lessac (Jarvis-like)" },
            { value: "english_ryan", label: "Male — Ryan" },
            { value: "english_amy", label: "Female — Amy" },
          ]}
        />
        <SelectRow
          label="Style"
...
function SystemSection() {
  const {
    ...
    gmailSpeakAlerts, setGmailSpeakAlerts,
    gmailNotifyLevel, setGmailNotifyLevel,
  } = useSettingsStore();
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── Gmail Integration ── */}
      <SettingsGroup title="Gmail Integration">
        <ToggleRow
          label="Enable Gmail Monitoring"
          description="NOVA watches your inbox and alerts you like Jarvis."
          value={gmailEnabled}
        ...
        {gmailEnabled && (
          <>
            <ToggleRow
              label="Speak Email Alerts"
              description="NOVA announces new emails via voice."
              value={gmailSpeakAlerts}
              onChange={setGmailSpeakAlerts}
            />
            <div className="flex items-center justify-between py-2">
              <div>
                <p className="text-[13px] font-semibold text-text-primary">Notify For</p>
                <p className="text-[11px] text-text-muted mt-0.5">Minimum urgency level to announce.</p>
              </div>
              <select
                className="bg-bg-elevated border border-border text-text-primary text-[12px] rounded-lg px-3 py-1.5 focus:outline-none focus:border-nova-orange"
                value={gmailNotifyLevel}
                onChange={(e) => setGmailNotifyLevel(e.target.value as "all" | "high" | "critical")}
              >
            ...
            <div className="rounded-lg bg-bg-elevated border border-border p-3 text-[11px] text-text-muted space-y-1">
              <p className="font-semibold text-text-primary text-[12px]">Setup Required</p>
              <p>1. Enable Gmail API in <span className="text-[#38bdf8]">Google Cloud Console</span></p>
              <p>2. Download <code className="bg-bg px-1 rounded">credentials.json</code> (Desktop OAuth)</p>
              <p>3. Set <code className="bg-bg px-1 rounded">GMAIL_CREDENTIALS_PATH</code> in your <code className="bg-bg px-1 rounded">.env</code></p>
              <p>4. Set <code className="bg-bg px-1 rounded">GMAIL_MONITOR_ENABLED=true</code> and restart the backend</p>
              <p>5. First run opens browser for Google OAuth consent → token saved automatically</p>
            </div>
          </>
        )}
      </SettingsGroup>
    ...
    <div className="flex items-center justify-between gap-4 py-3 border-b border-border/50 last:border-0">
      ...
      <button
        ...
        className={clsx(
          ...
          value
            ? "bg-nova-orange/90 border-nova-orange shadow-[0_0_0_2px_rgba(249,115,22,0.22)]"
  ...
  return (
    <div className="flex items-center justify-between gap-4 py-3 border-b border-border/50 last:border-0">
      ...
      <select
        ...
        className="bg-bg-elevated border border-border rounded-lg px-3 py-1.5 text-[12px] text-text-primary outline-none focus:border-nova-orange/50 max-w-[200px]"
      >
        {options.map((o) => (
